...
/** Google Places API Table A types grouped by category (excludes geographical/admin types). */
...
export const PLACE_CATEGORY_GROUPS: PlaceCategoryGroup[] = Object.entries(RAW_GROUPS).map(
  ([group, types]) => ({
    group,
    categories: types.map((id) => ({
      id,
      label: formatPlaceTypeLabel(id),
    })),
  })
);

export const ALL_PLACE_CATEGORIES: PlaceCategory[] = PLACE_CATEGORY_GROUPS.flatMap(
  (g) => g.categories
);

export const DEFAULT_CATEGORY_ID = "plumber";

export function getPlaceCategory(id: string): PlaceCategory | undefined {
  return ALL_PLACE_CATEGORIES.find((c) => c.id === id);
}
